...
1. В установленной при выполнении предыдущей лабораторной работыоперационной системе создайте учётную запись пользователя guest (использую учётную запись администратора): useradd guest ([рис. 0](pic/0.png)) ([рис. 1](pic/1.png))
![Вход в сиситему](pic/0.png){ #fig:001 }

![Окно терминала «Сооздание нового пользователя guest»](pic/1.png){ #fig:002 }

2. Задайте пароль для пользователя guest (использую учётную запись администратора): passwd guest([рис. 2])

![Создание пароля для нового пользоваетеля](pic/2.png){ #fig:003 }

3. Войдите в систему от имени пользователя guest.([рис. 3])

![Вход в систему](pic/3.2.PNG){ #fig:004 }
...
![Определение директории](pic/4.PNG){ #fig:005 }

5. Уточните имя вашего пользователя командой whoami ([рис. 5](pic/5.png))

![Имя пользователя](pic/5.PNG){ #fig:006 }
...
![Имя, группы пользователя](pic/6.PNG){ #fig:007 }
...
![сat/etc/passwd | grep guest](pic/9.PNG){ #fig:010 }
